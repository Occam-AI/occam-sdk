...
class OAuthConfig(BaseModel):
    ...
    flow_type: StrictStr
    provider: StrictStr
    __properties: ClassVar[List[str]] = ["required", "flow_type", "provider"]

    @field_validator('required')
    def required_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['always', 'optional']):
            raise ValueError("must be one of enum values ('always', 'optional')")
        return value

    @field_validator('flow_type')
    def flow_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['redirect', 'isolated']):
            raise ValueError("must be one of enum values ('redirect', 'isolated')")
        return value
